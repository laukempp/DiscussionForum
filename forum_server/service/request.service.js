...
function createTopic(req, callback) {
    pool.connect((err, client) => {
        if (err) throw err;
        client.query('INSERT INTO topic(nickname, title, input) VALUES($1, $2, $3)',
                [req.body.nickname, req.body.title, req.body.input], (err, data) => {
            if (err) throw err;
            client.release();
            callback();
        });
...
function updateTopic(req, res, callback) {
    pool.connect((err, client) => {
        if (err) throw err;
        client.query('UPDATE topic SET nickname = $1, title = $2, input = $3 WHERE id = $4',
            [req.body.nickname, req.body.title, req.body.input, parseInt(req.params.id)], (err, data) => {
                if (err) throw err;
                client.release();
                res.json(req.body)
                callback(res.body);
